"""
End-to-end integration test: build a small synthetic dataset, train 1 epoch, 
and verify the full pipeline (data loading, model training, checkpointing, inference).

This test does NOT require the real PlantVillage dataset.
Run with: python .\tests\test_e2e.py
"""
import tempfile
...
def test_e2e():
    """Full pipeline: data -> model -> train 1 epoch -> checkpoint -> infer."""
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        train_loader, val_loader, loaded_classes = make_dataloaders(
            data_dir, batch_size=2, img_size=224, num_workers=0
        )
